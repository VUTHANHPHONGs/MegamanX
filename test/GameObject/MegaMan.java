...
import static GameObject.ParticularObject.TELE;
...
public class MegaMan extends Human {
    ...
    public MegaMan(float x, float y, GameWorld gameWorld) {
        super(x, y, 70, 90, 0.1f, 200, gameWorld);
        ...
        teleForwardAnim = CacheDataLoader.getInstance().getAnimation("tele");
        teleBackAnim = CacheDataLoader.getInstance().getAnimation("tele");
        teleBackAnim.flipAllImage();
...
//                            
            ...
            case TELE:
                ...
                teleForwardAnim.draw((int) (getPosX() - getGameWorld().camera.getPosX()), (int) getPosY() - (int) getGameWorld().camera.getPosY(), g2);
                break;
    ...
    public void specialAttack(){
        if(!isShooting && !getIsDicking()){
            ...
            Bullet bullet = new SpecialBullet(getPosX(), getPosY(), getGameWorld());
            if(getDirection() == LEFT_DIR) {
                bullet.setSpeedX(-10);
                bullet.setPosX(bullet.getPosX() - 40);
                if(getSpeedX() != 0 && getSpeedY() == 0){
