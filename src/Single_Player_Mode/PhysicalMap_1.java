 ...
 * @author Admin
 */
public class PhysicalMap_1 extends GameObject_1{
    public int[][] phys_map;
    private int tileSize;

    public PhysicalMap_1(float x,float y,GameWorld_1 gameWorld){
        super(x,y, gameWorld);
        this.tileSize=30;
    ...
    public void draw(Graphics2D g2){
        ...
        for(int i=0;i<phys_map.length;i++){
            for(int j=0;j<phys_map[0].length;j++){
                if(phys_map[i][j] !=0) g2.fillRect((int)getPosX()+ j*tileSize-(int)camera.getPosX(), (int) getPosY() +i*tileSize-(int) camera.getPosY(), tileSize,tileSize);
            }
        }
    }
    //kiemtravachamvoi mat dat
